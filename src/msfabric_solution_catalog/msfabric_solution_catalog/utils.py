...
CREDENTIAL_OVERRIDE_ENV_VAR = "MSFABRIC_SOLUTION_CATALOG_TOKEN_CREDENTIAL"
...
def resolve_token_credential():
    ...
    Resolution order:
    1. ``MSFABRIC_SOLUTION_CATALOG_TOKEN_CREDENTIAL`` environment variable
...
def create_working_directory(temp_dir_prefix: str = "msfabric-solution-catalog-") -> Path:
    dest_dir = tempfile.mkdtemp(prefix=temp_dir_prefix)
    return Path(dest_dir)

def clone_files_to_temp_directory(
    source_path: Path,
    temp_dir_prefix: str = "msfabric-solution-catalog-"
):
    """
...
def update_docs_uri_with_ref(
    ...
    Args:
        docs_uri: The original catalog_docs_uri from config
