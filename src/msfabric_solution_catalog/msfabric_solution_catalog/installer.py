"""catalog installer orchestration."""
...
class catalogInstaller:
    """Orchestrates the installation of a catalog to a Fabric workspace."""
    ...
    def __init__(
        ...
        **options
        ...
        Args:
            config: catalog configuration dictionary from registry
            workspace_id: Target workspace GUID
            instance_name: Variable name of catalog instance
    ...
    def effective_docs_uri(self) -> Optional[str]:
        ...
        docs_uri = self.config.get('catalog_docs_uri')
    ...
    def prepare_workspace(self) -> Path:
        ...
        if config_id is None:
            raise ValueError("catalog config missing required 'id' field")
        ...
        if 'repo_url' in source_config:
            ...
            repo_path = catalogs_dir / logical_id
    ...
    def generate_entry_url(self, target_ws: FabricWorkspace, prefix: Optional[str]) -> Optional[str]:
        """Generate entry point URL for deployed catalog.
