"""Render install status displays for Fabric catalog."""
...
def render_install_status_html(*, status: str, catalog_name: str, type: str, workspace_id: Optional[str], entry_point, minutes_complete, minutes_deploy, docs_uri=None, logs=None, error_message: Optional[str] = None, extra_html: Optional[str] = None, elapsed_seconds: float = 0.0, progress_override: Optional[float] = None):
    ...
    safe_name = html.escape(catalog_name or 'catalog', quote=True)
    ...
    if docs_uri:
        ...
        top_items.append(
            f'<div class="install-status-item"><div class="install-status-label">catalog documentation</div><div class="install-status-value">{docs_markup}</div></div>'
        )
    ...
    if status_lower == 'success':
        ...
        eta_str = '⏱ Est. time to complete catalog'
        if type == 'accelerator':
