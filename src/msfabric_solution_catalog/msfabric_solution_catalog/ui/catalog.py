"""Catalog rendering functions for Fabric catalog."""
...
_catalog_CSS = _load_text(_css_path)
_catalog_JS = _load_text(_js_path)


def reload_assets():
    """Reload CSS/JS assets from disk without restarting the kernel."""
    global _catalog_CSS, _catalog_JS
    _catalog_CSS = _load_text(_css_path)
    _catalog_JS = _load_text(_js_path)
...
# Map catalog types to quick-recognition emojis for the card tag
TYPE_EMOJI_MAP = {
    "Accelerator": "🚀",
    "Demo": "▶️",
    "Tutorial": "📓",
...
def _resolve_workload_colors(catalog, category_tag=None, group_by="scenario"):
    ...
    workload_tags = catalog.get("workload_tags") or []
...
def render_catalog_list(grouped_scenario, grouped_workload, grouped_type, instance_name):
    """
    Generate HTML UI for catalogs listing with interactive toggle and tag filters.
    
    Args:
        grouped_scenario: Dictionary of catalogs grouped by scenario tags
        grouped_workload: Dictionary of catalogs grouped by workload tags
        instance_name: The variable name of the catalog instance
...
def _generate_html(grouped_scenario, grouped_workload, grouped_type, scenario_tags, workload_tags, type_tags, instance_name):
    # Arc catalog theming - load from external assets
    style = f"<style>{_catalog_CSS}</style>" if _catalog_CSS else ""
    script = f"<script>{_catalog_JS}</script>" if _catalog_JS else ""

    # Build HTML
    html_parts = [style, script, '<div class="catalog-container">']
    
    # Header with Arc catalog styling
    html_parts.append('''
        <div class="catalog-header">
            <div class="catalog-label">DISCOVER</div>
    ...
    html_parts.append(_render_grouped_catalogs(grouped_scenario, instance_name, group_by="scenario"))
    html_parts.append('</div>')
    
    # Workload view
    html_parts.append('<div id="workload-view" class="view-container active">')
    html_parts.append(_render_grouped_catalogs(grouped_workload, instance_name, group_by="workload"))
    html_parts.append('</div>')

    # Type view
    html_parts.append('<div id="type-view" class="view-container">')
    html_parts.append(_render_grouped_catalogs(grouped_type or {}, instance_name, group_by="type"))
    html_parts.append('</div>')
...
def _render_grouped_catalogs(grouped_catalogs, instance_name, group_by="scenario"):
    """Render HTML for grouped catalogs with Arc catalog styling."""
    html_parts = []
    
    for category, catalogs_list in sorted(grouped_catalogs.items()):
        ...
        section_primary, section_secondary = _resolve_workload_colors(
            catalogs_list[0] if catalogs_list else {},
        ...
        for j in catalogs_list:
            # Generate card HTML
            new_badge = '<div class="catalog-new-badge">NEW</div>' if j.get('is_new') else ''
            ...
            class_badge = f'<div class="class-pill {"class-pill-core" if is_core else "class-pill-community"}" aria-label="catalog class: {class_label}">{class_label}</div>'
            ...
            complete_min_val = j.get('minutes_to_complete_catalog')
            meta_parts = []
            ...
            meta_footer_html = f'<div class="catalog-meta-footer">{meta_footer_text}</div>' if meta_footer_text else ''

            html_parts.append(f'''
                <div class="catalog-card" data-type="{type_value}" data-workloads="{workloads_value}" data-scenarios="{scenarios_value}">
                    <div class="catalog-image">{diagram_html}{new_badge}<div class="workload-ribbon">{workload_badges_html}</div></div>
                    <div class="catalog-content">
                        {meta_block}
                        <div class="catalog-name">{card_name}</div>
                        <div class="catalog-description" title="{description_title}">{description_text}</div>
