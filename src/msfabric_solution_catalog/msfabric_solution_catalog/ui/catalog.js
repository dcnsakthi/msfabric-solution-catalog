...
function initcatalogUI() {
    ...
    window.catalogFilters.active = ['workload', 'scenario', 'type'];
    window.catalogFilters.values = { workload: null, scenario: null, type: null };
...
function toggleView(viewType, clickedButton) {
    window.catalogFilters.activeView = viewType;
...
function toggleFilterMenu() {
    ...
    if (!window.catalogData || !window.catalogData.length) {
        collectData();
    }
...
function addFilter(kind) {
    if (!window.catalogFilters.active.includes(kind)) {
        window.catalogFilters.active.push(kind);
    }
    const options = getAvailableOptions(kind);
    window.catalogFilters.values[kind] = null; // Start with "- All -"
    renderFilters();
    applyFilters();
    closeFilterMenu();
}
...
function removeFilter(kind) {
    window.catalogFilters.active = window.catalogFilters.active.filter(k => k !== kind);
    window.catalogFilters.values[kind] = null;
    renderFilters();
    applyFilters();
}

function updateFilter(kind, value) {
    window.catalogFilters.values[kind] = (value === '' || !value) ? null : value;
    renderFilters();
    applyFilters();
}
...
function renderFilterMenu() {
    ...
    menu.innerHTML = '';
    const remaining = ['type','workload','scenario'].filter(k => !window.catalogFilters.active.includes(k));
    let added = 0;
...
function renderFilters() {
    ...
    container.innerHTML = '';
    window.catalogFilters.active.forEach(kind => {
        ...
        const options = getAvailableOptions(kind);
        if (window.catalogFilters.values[kind] && !options.includes(window.catalogFilters.values[kind])) {
            window.catalogFilters.values[kind] = null;
        }
        if (!options.length) {
            ...
            if (!window.catalogFilters.values[kind]) {
            ...
            options.forEach(opt => {
                ...
                o.value = opt;
                o.textContent = opt;
                if (window.catalogFilters.values[kind] === opt) {
                    o.selected = true;
                }
...
function applyFilters() {
    const viewType = window.catalogFilters.activeView || 'workload';
    const filters = window.catalogFilters.values;
    ...
    if (!anyVisible) {
        if (!notice && container) {
            ...
            notice.textContent = 'No catalogs match the current filters.';
...
window.initcatalogUI = initcatalogUI;
