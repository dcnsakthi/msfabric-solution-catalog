"""Lightweight telemetry for catalog install events.
...
``catalog_TELEMETRY_OPTOUT=1``.  The connection string can be
...
_USER_HASH_SALT = "msfabric-solution-catalog-telemetry-v1"
...
def _get_connection_string() -> Optional[str]:
    ...
    if os.environ.get("catalog_TELEMETRY_OPTOUT", "").strip() in ("1", "true", "yes"):
        return None
    return (
...
def _track_install_worker(
    ...
    properties: dict[str, str] = {
        ...
        "catalog_numeric_id": str(catalog_numeric_id),
        "type": catalog_type,
        "status": status,
    }
...
def track_install(
    ...
    """Record a catalog install event (fire-and-forget).
    ...
    try:
        ...
        t = threading.Thread(
            ...
            args=(conn_str, catalog_id, catalog_numeric_id, catalog_type, status),
