"""Tests for repo_ref override in catalogInstaller."""

from unittest.mock import patch, MagicMock
from msfabric_solution_catalog.installer import catalogInstaller
from msfabric_solution_catalog.utils import update_docs_uri_with_ref


def _make_config(**overrides):
    """Return a minimal catalog config dict."""
    config = {
        "id": 1,
...
@patch("msfabric_solution_catalog.installer.clone_repository")
def test_prepare_workspace_uses_config_repo_ref_by_default(mock_clone):
    ...
    installer = catalogInstaller(_make_config(), workspace_id="ws-123", instance_name="js")
...
def test_effective_docs_uri_returns_original_when_no_override():
    ...
    config = _make_config(catalog_docs_uri="https://github.com/example/repo/blob/v1.0.0/README.md")
    installer = catalogInstaller(config, workspace_id="ws-123", instance_name="js")
...
def test_effective_docs_uri_preserves_non_github_urls():
    ...
    config = _make_config(catalog_docs_uri="https://catalog.fabric.microsoft.com/docs/")
    installer = catalogInstaller(
        config, workspace_id="ws-123", instance_name="js", repo_ref="v2.0.0"
    )
    assert installer.effective_docs_uri == "https://catalog.fabric.microsoft.com/docs/"


def test_effective_docs_uri_handles_empty_string():
    """Empty docs_uri is returned unchanged."""
    config = _make_config(catalog_docs_uri="")
    installer = catalogInstaller(
        config, workspace_id="ws-123", instance_name="js", repo_ref="v2.0.0"
    )
...
def test_effective_docs_uri_handles_none():
    ...
    config = _make_config()  # No catalog_docs_uri
    installer = catalogInstaller(
        config, workspace_id="ws-123", instance_name="js", repo_ref="v2.0.0"
    )
