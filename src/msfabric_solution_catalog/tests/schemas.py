"""Pydantic schemas for catalog registry validation (test-only)."""
...
from msfabric_solution_catalog.constants import (
...
class catalogSource(BaseModel):
    """Source configuration for a catalog."""
    workspace_path: str
...
class catalog(BaseModel):
    ...
    catalog_docs_uri: Optional[str] = None
    entry_point: str
    test_suite: Optional[str] = None
    owner_email: str
    minutes_to_complete_catalog: Optional[int] = None
    ...
    def validate_description_not_prefixed_by_name(self):
        ...
        if name and desc.lower().startswith(name.lower()):
            raise ValueError("description must not start with the catalog name")
        return self
    ...
    @classmethod
    def validate_catalog_type(cls, value: Optional[str]):
        if value is None:
            return "Accelerator"
        if value not in VALID_catalog_TYPES:
            allowed_display = ", ".join(VALID_catalog_TYPES)
            raise ValueError(
                f"Unknown catalog_type: {value}. Allowed values: {allowed_display}."
            )
        return value
    ...
    @field_validator("minutes_to_complete_catalog", "minutes_to_deploy", mode="before")
    @classmethod
