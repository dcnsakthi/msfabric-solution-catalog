...
from msfabric_solution_catalog.installer import catalogInstaller
from msfabric_solution_catalog.utils import upload_files_to_lakehouse
...
def _make_config(**source_overrides):
    """Return a minimal catalog config dict with optional source overrides."""
    source = {
...
class TestUploadFilesToLakehouse:
    ...
    @patch("msfabric_solution_catalog.utils._is_fabric_runtime", return_value=False)
    @patch("msfabric_solution_catalog.utils.resolve_token_credential")
    @patch("msfabric_solution_catalog.utils.requests")
...
# ---------------------------------------------------------------------------
# Group 2: catalogInstaller.upload_files (installer.py)
# ---------------------------------------------------------------------------


class TestInstallerUploadFiles:
    """Tests for catalogInstaller.upload_files orchestration."""
    ...
    @patch("msfabric_solution_catalog.installer.upload_files_to_lakehouse", return_value=3)
    @patch(
    ...
    def test_applies_prefix_to_lakehouse_name(
        ...
        installer = catalogInstaller(config, workspace_id="ws-1", instance_name="js")
    ...
    @patch("msfabric_solution_catalog.installer.upload_files_to_lakehouse", return_value=1)
    @patch(
        "fabric_cicd._parameter._utils._extract_item_attribute",
    ...
    @patch("msfabric_solution_catalog.installer.upload_files_to_lakehouse", return_value=2)
    @patch(
