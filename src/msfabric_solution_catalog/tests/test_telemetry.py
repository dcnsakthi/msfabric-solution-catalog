...
from msfabric_solution_catalog.telemetry import (
    _USER_HASH_SALT,
    _build_envelope,
    _hash_user_id,
    _resolve_user_hash,
...
class TestResolveUserHash:
    ...
    @patch("msfabric_solution_catalog.utils._decode_jwt")
...
class TestTrackInstallWorker:
    ...
    @patch("msfabric_solution_catalog.telemetry._send")
    @patch("msfabric_solution_catalog.telemetry._resolve_user_hash")
    ...
    def test_does_not_send_without_ikey(self, mock_resolve, mock_send):
        ...
        _track_install_worker(
            conn_str=conn_str,
            catalog_id="test-js",
            catalog_numeric_id=1,
            catalog_type="Tutorial",
            status="success",
        )
