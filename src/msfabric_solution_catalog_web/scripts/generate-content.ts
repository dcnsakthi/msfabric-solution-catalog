 ...
 *
 * Reads scenario YAML files from core/ and community/ catalog directories
 * and generates:
...
const catalogS_BASE_DIR = path.resolve(
  __dirname,
  '../../msfabric_solution_catalog/msfabric_solution_catalog/catalogs'
);
const catalogS_CORE_DIR = path.join(catalogS_BASE_DIR, 'core');
const catalogS_COMMUNITY_DIR = path.join(catalogS_BASE_DIR, 'community');
...
function loadScenarios(): TaggedScenarioYml[] {
  const core = loadScenariosFromDir(catalogS_CORE_DIR, true);
  const community = loadScenariosFromDir(catalogS_COMMUNITY_DIR, false);
...
const SAMPLE_IMAGE_SRC = path.resolve(
  __dirname,
  '../../../.temp/msfabric-solution-catalog-web/msfabric-solution-catalog-docs/docs/drops_contribution_guidelines/progress_bar.png'
);
...
function generateContentMd(scenario: ScenarioYml): string {
  ...
  md += `This catalog deploys a fully functional ${title.toLowerCase()} scenario into your Microsoft Fabric workspace. `;
...
function generateStaticDocs(rootDocsDir: string): void {
  ...
  fs.writeFileSync(
    ...
    '---\ntype: docs\ntitle: "Getting Started"\nlinkTitle: "Getting Started"\nweight: 0\ndescription: >-\n  Learn the prerequisites for using a catalog, including creating a Fabric workspace, creating a notebook, and installing the msfabric-solution-catalog library.\n---\n'
  );
...
function isNewcatalog(dateAdded: string): boolean {
  try {
  ...
  return scenarios
    ...
    .map((s) => {
      ...
      return {
        ...
        minutesToComplete: s.minutes_to_complete_catalog,
        itemsInScope: s.items_in_scope,
        docsUri: s.catalog_docs_uri,
        slug: s.logical_id,
        lastUpdated: s.last_updated || s.date_added,
        body,
        core: s._core,
        isNew: isNewcatalog(s.date_added),
...
async function fetchInstallCounts(): Promise<Record<string, number>> {
  ...
  const query = [
    'customEvents',
    '| where name == "catalog_installed"',
    '| extend catalog_id = tostring(customDimensions.catalog_id),',
    '         status = tostring(customDimensions.status)',
    '| where status == "success"',
    '| summarize install_count = count() by catalog_id',
  ].join('\n');
  ...
  try {
    ...
    const idIdx = table.columns.findIndex((c) => c.name === 'catalog_id');
    ...
    console.log(`  Fetched install counts for ${Object.keys(counts).length} catalogs`);
    return counts;
  } catch (err) {
...
// Mirrors WORKLOAD_COLOR_MAP from msfabric_solution_catalog/constants.py
