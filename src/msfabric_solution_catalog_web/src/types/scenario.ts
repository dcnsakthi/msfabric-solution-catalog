/**
 * Canonical type definitions for catalog scenario data.
 *
 ...
 * ScenarioYml    – top-level shape of catalogs/core/*.yml
 * ScenarioCard   – shape emitted to scenarios.json for UI components
 *
 * Both Python (Pydantic catalog model) and TypeScript share the same YAML
 * contract.  If you add a field to the YAML, add it here AND to the Python
 * model in src/msfabric_solution_catalog/tests/schemas.py.
 */
...
export class ScenarioYml {
  ...
  catalog_docs_uri: string = '';
  entry_point: string = '';
  owner_email: string = '';
  minutes_to_deploy: number = 0;
  minutes_to_complete_catalog: number = 0;
