...
Thank you for helping improve Fabric catalog!

## Development Setup to contribute to all sub-projects (e.g. Python CLI, Website)

### Windows users

[![catalog Walkthrough](../../.imgs/catalog-walkthrough.png)](https://catalogfabric.blob.core.windows.net/public/catalog-dev-env-setup.mp4)
...
```bash
./src/msfabric_solution_catalog_web/start-dev-server.sh
```
...
```bash
./src/msfabric_solution_catalog_web/start-dev-server.sh 3000
```
...
This reads the `mermaid_diagram` field from every catalog YAML and writes SVGs to `assets/images/diagrams/`.
