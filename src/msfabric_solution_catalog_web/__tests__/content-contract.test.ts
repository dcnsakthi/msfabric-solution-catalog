...
const catalogS_CORE_DIR = path.resolve(
  __dirname,
  '../../msfabric_solution_catalog/msfabric_solution_catalog/catalogs/core'
);
const catalogS_COMMUNITY_DIR = path.resolve(
  __dirname,
  '../../msfabric_solution_catalog/msfabric_solution_catalog/catalogs/community'
);
...
/** Return the set of logical_ids from catalog YAML files (core + community). */
function loadLogicalIds(): Set<string> {
  const ids = new Set<string>();
  for (const dir of [catalogS_CORE_DIR, catalogS_COMMUNITY_DIR]) {
...
describe('Scenario content contract', () => {
  ...
  // ── folder name matches a catalog logical_id ─────────────

  test.each(dirs)(
    'scenario "%s" matches a catalog logical_id',
    (dir) => {
      const ids = loadLogicalIds();
      if (!ids.has(dir)) {
        throw new Error(
          `Content folder "${dir}" does not match any logical_id in catalogs/core/ or catalogs/community/.`
        );
      }
    }
  );
