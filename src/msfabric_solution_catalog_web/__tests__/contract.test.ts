 ...
 *
 * Ensures all scenarios in catalogs/core/ conform to the expected contract
...
const catalogS_DIR = path.resolve(
  __dirname,
  '../../msfabric_solution_catalog/msfabric_solution_catalog/catalogs/core'
);

function loadAllScenarios(): { file: string; data: ScenarioYml }[] {
  const files = glob.sync('*.yml', { cwd: catalogS_DIR });
  return files.map((file) => {
    const content = fs.readFileSync(path.join(catalogS_DIR, file), 'utf-8');
...
describe('Scenario YAML contract', () => {
  ...
  test.each(scenarios)('$file has valid types for base fields', ({ data }) => {
    ...
    expect(typeof data.minutes_to_complete_catalog).toBe('number');
  });
