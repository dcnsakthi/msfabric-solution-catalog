...
This catalog deploys a complete **bronze → silver → gold** medallion pipeline using materialized lake views in a Microsoft Fabric lakehouse. Run the tutorial notebook to create source tables, define materialized views with automatic refresh, and explore lineage tracking.
...
After installing the catalog, open the `mlv_get_started` notebook and run each cell in order. The notebook is fully self-contained and creates all schemas, tables, and materialized views.

## Architecture

The catalog implements the medallion architecture with three layers:
